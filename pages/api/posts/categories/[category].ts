...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (typeof category !== 'string') {
      sendBadRequest(res, 'Invalid category parameter');
      return;
    }

    if (req.method === 'GET') {
      await handleGetPostsByCategory(category, { published, limit, offset, sortBy, sortOrder, ...otherFilters }, res);
    } else {
...
async function handleGetPostsByCategory(
  category: string,
  queryParams: any,
  res: NextApiResponse
) {
